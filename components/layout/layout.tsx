...
const AdminLinkItems: Array<LinkItemProps> = [
    ...
    {name: 'Quản Lý Tình Nguyện Viên', icon: MdHistory, href: '/admin/volunteers'},
];
...
const SidebarContent = ({onClose, ...rest}: SidebarProps) => {
    ...
    return (
        <Box
            ...
            {isUser && LinkItems.map((link) => (
                <NavItem key={link.name} icon={link.icon} href={link.href}>
                    {link.name}
                </NavItem>
            ))}
            {isAdmin && AdminLinkItems.map((link) => (
                <NavItem key={link.name} icon={link.icon} href={link.href}>
                    {link.name}
                </NavItem>
            ))}
...
const MobileNav = ({onOpen : onOpen, ...rest}: MobileProps) => {
    const router = useRouter();
    const dispatch = useDispatch();
                ...
                <Flex alignItems={'center'}>
                    <Menu>
                        ...
                        <MenuList
                            ...
                            {isAdmin && <MenuItem>Cổng quản trị</MenuItem>}
